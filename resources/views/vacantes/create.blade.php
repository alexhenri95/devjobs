...
@section('content')
	<h1 class="text-2xl text-gray-900 text-center mt-10">Crear nueva vacante</h1>
	...
	<form action="{{route('vacantes.store')}}" method="POST" class="max-w-lg mx-auto my-10">
		...
		<div class="flex flex-wrap mb-5">
			...
			<input id="titulo" type="text" class="p-2 bg-gray-400 rounded form-input w-full @error('titulo') border-red-500 border @enderror" name="titulo" value="{{ old('titulo') }}"  autocomplete="titulo" autofocus>
		...
		<div class="flex flex-wrap mb-5">
			...
			<select name="sueldo" id="sueldo" class="block appearance-none w-full border border-gray-200 text-gray-700 rounded leading-tight focus:outline-none focus:bg-white focus:border-gray-500 p-3 bg-gray-400">
				...
				@foreach($sueldos as $sueldo)
					<option value="{{$sueldo->id}}" {{old('sueldo') == $sueldo->id ? 'selected':''}}>{{$sueldo->nombre}}</option>
				@endforeach
			</select>
		...
		<button type="submit" class="bg-indigo-500 w-full hover:bg-indigo-600 text-gray-100 p-3 focus:outline-none focus:shadow-outline uppercase font-bold text-sm cursor-pointer">Publicar vacante</button>
